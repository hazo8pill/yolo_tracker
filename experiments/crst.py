...
TARGET_CLASS = 2        # e.g., 2 = car
CONF_THRESHOLD = 0.25
AUTO_REDETECT_INTERVAL = 60  # frames between automatic YOLO refresh

# --- Multi-Tracker Fusion ---
class MultiTrackerFusion:
    def __init__(self):
        self.trackers = []
        self.names = ['CSRT', 'KCF', 'MOSSE']
    ...
    def update(self, frame):
        """Update trackers, fuse results (mean box of successful trackers)."""
        boxes = []
        ...
        fused_box = boxes.mean(axis=0)
        return True, tuple(fused_box)
        ...
        if tracker is None or auto_redetect:
            ...
            if best_box is not None:
                ...
                cv2.putText(frame, f"YOLO init (conf={best_conf:.2f})",
                            ...
                            (0, 255, 0), 2)
                print(f"Tracker initialized at {bbox_xywh} (conf={best_conf:.2f})")
